...
func main() {
	...
	apiClient := dnsconfig.NewAPIClient()
	resp, r, err := apiClient.ServerAPI.List(context.Background()).Fields(fields).Filter(filter).Offset(offset).Limit(limit).PageToken(pageToken).OrderBy(orderBy).Tfilter(tfilter).TorderBy(torderBy).Inherit(inherit).Execute()
	if err != nil {
